...
async fn get_ad_data(link: &str) -> Value {
    ...
    let result = client
        ...
        .send()
        .await
        .expect("Failed to get ad data");
    let text_result = result.text().await.expect("Failed to get ad data");
...
fn get_data_json(xml: &str) -> String {
    ...
    let parsed_result = &result.expect("Regex got no results")[0];
    let begin = 11;
...
fn filter_search_results(search_results: &OlxResults) -> Vec<String> {
    println!(
        ...
        search_results
            ...
            .iter()
            .nth(0)
            .expect("Failed to get first url")
            .url
    );
