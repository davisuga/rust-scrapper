...
async fn main() -> Result<(), ()> {
    let links = vec!["https://mg.olx.com.br/belo-horizonte-e-regiao/computadores-e-acessorios/monitor-curvo-samsung-24-fhd-hdmi-vga-freesync-preto-serie-cf390-853157530", "https://mg.olx.com.br/belo-horizonte-e-regiao/computadores-e-acessorios/monitor-27-philips-widescreen-4k-uhd-ips-mostruario-nota-garantia-838877513"];
...
fn parse_html(document_string: String) {
    ...
    let mut nodes = script_tags
        .map(|tag| {
            let json = tag.attr("data-json").unwrap();
            println!("{}", json);
            let parsed: serde_json::Result<Value> = serde_json::from_str(json);
